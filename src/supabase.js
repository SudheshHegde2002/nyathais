...
const DEFAULT_ORDER_ITEMS = [
  { id: 'item-1', order_id: 'order-1001-uuid', flavor_id: 'flavor-belgian-biscoff', quantity: 20 },
  { id: 'item-2', order_id: 'order-1001-uuid', flavor_id: 'flavor-mango', quantity: 15 },
  { id: 'item-3', order_id: 'order-1002-uuid', flavor_id: 'flavor-avocado', quantity: 30 },
  { id: 'item-4', order_id: 'order-1002-uuid', flavor_id: 'flavor-chilly-guava', quantity: 10 },
  { id: 'item-5', order_id: 'order-1002-uuid', flavor_id: 'flavor-red-fig', quantity: 15 },
  { id: 'item-6', order_id: 'order-1003-uuid', flavor_id: 'flavor-belgian-biscoff', quantity: 10 },
  { id: 'item-7', order_id: 'order-1003-uuid', flavor_id: 'flavor-tender-coconut', quantity: 25 }
];
...
export const supabaseService = {
  ...
  getFlavors: async (onlyActive = true) => {
    if (isUsingRealSupabase) {
      ...
      const db = getLocalDB();
      return onlyActive ? db.flavors.filter(f => f.active) : db.flavors;
    }
  },
  ...
  updateOrderStatus: async (orderId, status) => {
    if (isUsingRealSupabase) {
      ...
      // Broadcast update for realtime Simulation
      if (broadcastChannel) {
        broadcastChannel.postMessage({ event: 'UPDATE', table: 'orders', record: db.orders[index] });
      }
